...
void save_time(int k, double time_taken )
{
    std::ofstream MyFile;
    MyFile.open ("../data/queries/variable_k_nearest_neighbor_search_timings.txt", std::ios_base::app) ;
...
int nn_search(std::vector<circle_list_t>& vp, float qx, float qy,  int n, int k,std::priority_queue<HeapItem>& heap, int path)
{
...
double begin = omp_get_wtime();
	...
	while (query_size--)
	{
        ...
        int path =  nn_search(vp,query[i].x,query[i].y,vp_data_size,k,heap,0);
