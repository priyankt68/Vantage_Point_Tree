...
/*
float quick_select(std::vector<float>& distance_vector, std::vector<point>& pts, int from, int to, int k)
{
    int n = from + to -1 ;
 ...
 float pivot = distance_vector[r];
  ...
  while (r < w) {
   ...
   if (distance_vector[r] >= pivot) { // put the large values at the end
            swap(  &distance_vector[r] , &distance_vector[w]);
            swap(            &pts[r].x , &pts[w].x          );
            swap(            &pts[r].y , &pts[w].y          );
    w--;
...
{
    std::ofstream MyFile;
    MyFile.open ("../../data/vp_tree_build_for_point_search.txt",std::ofstream::out | std::ofstream::trunc) ;
...
int main()
{
  ...
  // print_input_data(points,n);                    /* Printing input data */
   ...
   // save_build_timing(n,time_spent);

    std :: cout << n << " " << (time_spent) << std :: endl;
